"""Regression test: release-asset data_cache must hold the expected USDC perp CSVs.
...
before release-asset drift reaches users.

Run from repo root:
    python3 -m unittest discover -s moss-trade-bot-factory/scripts/tests -p 'test_data_cache_complete.py'
"""
...
class DataCacheCompleteTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        global DATA_CACHE
        DATA_CACHE = str(ensure_data_cache())
