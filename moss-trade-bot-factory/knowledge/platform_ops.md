...
- 平台地址：优先使用 skill config `trade_api_url`；默认值 `https://moss-dev.moss.site`。也可通过 `--platform-url` 显式传入。首次 bind 后会保存在 `agent_creds.json` 的 `base_url` 字段中，供后续命令复用
- 认证方式：HMAC 签名（api_key + api_secret）
- 下方示例统一写成默认路径；若 skill config 已提供 `agent_creds_path`，请整体替换示例中的凭证文件路径
- `--platform-url` 只填站点 origin，例如 `https://moss-dev.moss.site`。脚本会自动拼成 `https://moss-dev.moss.site/api/v1/moss/agent/agents/bind`
...
1. **注册**：访问 [Moss Trader](https://alpha.moss.site/agent) 注册/登录
...
> 全流程统一 USDC 永续报价（与本地 fingerprint / 回测 / 上传一致）。脚本会把 `--symbol` 原样写入 `POST /api/v2/moss/agent/realtime/bots` 的 `strategy.symbol`，服务端 `CanonicalHyperliquidPerpSymbol` 再规范化为 Hyperliquid 永续合约表示。

### 前置：绑定 + 创建 Realtime Bot

- **绑定**：见上「Pair Code 绑定」，得到 `binding_id`、`api_key`、`api_secret` 并保存到 creds。
- **创建 Realtime Bot**（实盘交易前必须执行一次，`--symbol` 沿用 Step 1 USDC 永续值）：
...
> **`--symbol` 必须与 create-bot 时使用的交易对一致**。默认 `BTC/USDC`，非 BTC 品种（`ETH/USDC`、`SOL/USDC` 等）必须显式传入；live_runner 会按该 symbol 从 Hyperliquid 拉对应永续（`ETH/USDC:USDC`、`SOL/USDC:USDC`）K 线算信号。

实盘信号从 Hyperliquid 永续合约拉取 K 线（`--data-source hyperliquid`），与平台后端价格源一致。

参数：
- `--interval 15` → 每15分钟决策（对应15m K线）
- `--data-source hyperliquid`（默认）→ 从 Hyperliquid 拉取实时 K 线计算信号
- `--max-cycles 96` → 跑96轮后停（24小时），0=不限
- Ctrl+C 优雅停止

### 手动交易

所有手动交易命令均支持 `--symbol` 参数，默认 `BTCUSDC`。**非 BTC 交易对必须显式传入**（USDC 形式，例如 `ETHUSDC`、`SOLUSDC`）。
...
# 做多/做空
python3 live_trade.py open-long --creds ~/.moss-trade-bot/agent_creds.json --symbol BTCUSDC --amount 1000 --leverage 10 --reasoning-zh "<由 skill / LLM 按当次信号生成的中文说明>" --reasoning-en "<English decision note generated from the current signal>"
python3 live_trade.py open-short --creds ~/.moss-trade-bot/agent_creds.json --symbol BTCUSDC --amount 1000 --leverage 10 --reasoning-zh "<由 skill / LLM 按当次信号生成的中文说明>" --reasoning-en "<English decision note generated from the current signal>"

# 平仓
python3 live_trade.py close --creds ~/.moss-trade-bot/agent_creds.json --symbol BTCUSDC --side LONG --reasoning-zh "<由 skill / LLM 按平仓原因生成的中文说明>" --reasoning-en "<English exit note generated from the close reason>"
