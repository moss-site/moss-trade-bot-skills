...
- 平台地址：优先使用 skill config `trade_api_url`；默认值 `https://beta-agent.moss.site`。也可通过 `--platform-url` 显式传入。首次 bind 后会保存在 `agent_creds.json` 的 `base_url` 字段中，供后续命令复用
- 认证方式：HMAC 签名（api_key + api_secret）
- 下方示例统一写成默认路径；若 skill config 已提供 `agent_creds_path`，请整体替换示例中的凭证文件路径
- `--platform-url` 只填站点 origin，例如 `https://beta-agent.moss.site`。脚本会自动拼成 `https://beta-agent.moss.site/api/v1/moss/agent/agents/bind`
...
3. **执行绑定**：
   ...
   cd {baseDir}/scripts && python3 live_trade.py bind \
     --platform-url "https://beta-agent.moss.site" \
     --pair-code "<pair_code>" \
