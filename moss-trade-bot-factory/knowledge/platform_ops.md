...
- 回测 / 上传验证使用预置的 Hyperliquid 固定数据集 CSV（`data_cache_manifest.json` / Release Asset 明确列出的 43 币；`scripts/data_cache/` 只是传参用逻辑路径，真实 CSV 首次运行后位于本地缓存）；上传前必须以 `dataset_catalog.py` 实际发现的 `csv_path/start/end/bars` 为准，不要使用旧的固定区间记忆
