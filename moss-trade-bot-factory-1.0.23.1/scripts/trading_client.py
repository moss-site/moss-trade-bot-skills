...
MIN_REASONING_ZH_LEN = 120
MIN_REASONING_EN_LEN = 120
...
class TradingClient:
    ...
    def _base_asset(symbol: str) -> str:
        """Strip quote suffix so BTCUSDT and BTCUSDC compare equal as the same underlying perp.

        The Moss platform stores positions under the perp's actual quote (e.g. BTCUSDC even
        when the bot was created with --symbol BTC/USDT), so client-side filtering must
        compare base assets, not the raw symbol string.

        Handles ccxt perp form 'BTC/USDC:USDC' (settle suffix), slash form 'BTC/USDT',
        compact form 'BTCUSDT', and a bare base 'BTC'.
        """
    ...
    def _symbols_match(a: str, b: str) -> bool:
        """True when two symbols name the same underlying perp regardless of USDT/USDC quote."""
        return TradingClient._base_asset(a) == TradingClient._base_asset(b)
    ...
    @staticmethod
    def _require_bilingual_reasoning(reasoning: str, reasoning_en: str) -> tuple[str, str]:
        """Hard gate: reasoning must be present in zh+en and pass bilingual rules."""
        zh = str(reasoning or "").strip()
        en = str(reasoning_en or "").strip()
        pair = validate_bilingual_text("reasoning", {"zh": zh, "en": en}, 512)
        if len(pair["zh"]) < MIN_REASONING_ZH_LEN:
            raise ValueError(
                f"reasoning.zh too short (min {MIN_REASONING_ZH_LEN}; target about 200 Chinese characters)"
            )
        if len(pair["en"]) < MIN_REASONING_EN_LEN:
            raise ValueError(
                f"reasoning.en too short (min {MIN_REASONING_EN_LEN}; write a full English decision note)"
            )
    ...
    def _resolve_open_position(self, position_side: str = "") -> dict:
        ...
        for position in positions:
            ...
            if not self._symbols_match(position_symbol, self.symbol):
                continue
    ...
    def _submit_market_order(
        ...
        *,
        ...
        reasoning_text, reasoning_en_text = self._require_bilingual_reasoning(reasoning, reasoning_en)
