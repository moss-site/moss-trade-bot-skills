---
name: moss-trade-bot-factory-1.0.24
...
- **交易品种**：从用户描述中提取。"交易ETH"→`ETH/USDC`，"做空SOL"→`SOL/USDC`，未提及具体币种→默认 `BTC/USDC`。本 skill 全流程统一使用 USDC 永续报价（与 Hyperliquid 后端、平台 API 一致）
  ...
  - **能否本地回测只看 `scripts/data_cache/` 是否有对应 CSV**。本 skill 已内置 **43 个 USDC 永续数据集**（2026-05-18 扩展），按类型分两组：
    - **HyperCore 主板（23 个，15m 固定数据集）**：BTC、ETH、SOL、BNB、DOGE、APT、ATOM、AVAX、BCH、DOT、FIL、HBAR、LINK、LTC、NEAR、OP、SUI、TRX、UNI、XRP、ADA、ARB、**HYPE**。其中 BTC 已刷新为 304d（2025-07-01 起），其余未刷新币种仍为 148d（2025-10-06 起）
...
- **BTC** → `2025-07-01 00:00:00 ~ 2026-04-30 23:45:00`（304 天，29184 bars），文件名 `hyperliquid_BTCUSDC_15m_2025-07-01_304d.csv`
- **主板其他币种 + 大多数 xyz 资产** → `2025-10-06 ~ 2026-03-03`（148 天，约 14K bars）
...
if [ "$COMPACT" = "BTCUSDC" ]; then
  DATA_CSV="{baseDir}/scripts/data_cache/hyperliquid_BTCUSDC_15m_2025-07-01_304d.csv"
fi
```
...
文件命名格式为 `hyperliquid_{COMPACT}USDC_15m_{dataset-version}.csv`（全部 USDC 永续；BTC 当前版本是 `2025-07-01_304d`，其他大多数币种仍是 `2025-10-06_148d`；xyz 资产文件名只用 base，例如 `hyperliquid_NVDAUSDC_15m_*.csv` 对应 HL 的 `xyz:NVDA`，后端 normalize 自动路由）。**运行时不接受用户传入外部 CSV**。
