...
Usage:
    ...
    # Open long:
    python live_trade.py open-long --key ak_xxx --secret as_xxx --bot-id agt_xxx --amount 1000 --leverage 10 --reasoning "<LLM基于当次信号生成的自然语言说明>"

    # Open short:
    python live_trade.py open-short --key ak_xxx --secret as_xxx --bot-id agt_xxx --amount 1000 --leverage 10 --reasoning "<LLM基于当次信号生成的自然语言说明>"

    # Close position:
    python live_trade.py close --key ak_xxx --secret as_xxx --bot-id agt_xxx --side LONG --reasoning "<LLM基于平仓原因生成的自然语言说明>"
...
def cmd_open_long(args):
    client = load_client(args)
    result = client.open_long(args.amount, args.leverage, args.order_id or "", args.reasoning or "")
    print(json.dumps(result, indent=2))


def cmd_open_short(args):
    client = load_client(args)
    result = client.open_short(args.amount, args.leverage, args.order_id or "", args.reasoning or "")
    print(json.dumps(result, indent=2))


def cmd_close(args):
    client = load_client(args)
    result = client.close_position(args.side, args.qty or "", args.order_id or "", args.reasoning or "")
    print(json.dumps(result, indent=2))
...
def main():
    ...
    p.add_argument("--reasoning", default="", help="Natural-language decision note generated by the skill/LLM and sent to backend")
    p.add_argument("--platform-url", default="", help=PLATFORM_URL_HELP + " Otherwise reuse base_url from creds file.")
    ...
    p.add_argument("--reasoning", default="", help="Natural-language exit note generated by the skill/LLM and sent to backend")
    p.add_argument("--platform-url", default="", help=PLATFORM_URL_HELP + " Otherwise reuse base_url from creds file.")
