...
def load_client(args) -> TradingClient:
    symbol = getattr(args, "symbol", "BTCUSDC") or "BTCUSDC"
...
def resolve_reasoning(args):
    """Reasoning is required + bilingual. No legacy single-language fallback."""
    reasoning_zh = getattr(args, "reasoning_zh", "")
    reasoning_en = getattr(args, "reasoning_en", "")
    try:
        reasoning_zh, reasoning_en = TradingClient._require_bilingual_reasoning(reasoning_zh, reasoning_en)
    except ValueError as exc:
        print(f"Error: {exc}", file=sys.stderr)
        print(
            "Hint: --reasoning-zh and --reasoning-en are both required. "
            "zh must contain Chinese characters and be about 200 Chinese characters; "
            "en must be English only and sufficiently detailed.",
            file=sys.stderr,
        )
        sys.exit(2)
    return reasoning_zh, reasoning_en
...
def main():
    ...
    p.add_argument("--reasoning-zh", required=True, help="Chinese decision note sent as reasoning (required, must contain 汉字)")
    p.add_argument("--reasoning-en", required=True, help="English decision note sent as reasoning_en (required, English only)")
    p.add_argument("--platform-url", default="", help=PLATFORM_URL_HELP + " Otherwise reuse base_url from creds file.")
    ...
    p.add_argument("--reasoning-zh", required=True, help="Chinese exit note sent as reasoning (required, must contain 汉字)")
    p.add_argument("--reasoning-en", required=True, help="English exit note sent as reasoning_en (required, English only)")
    p.add_argument("--platform-url", default="", help=PLATFORM_URL_HELP + " Otherwise reuse base_url from creds file.")
