...
def _ambush_params_for_wire(p: dict) -> dict:
    ...
    NOTE: trigger thresholds (oi_mc, z_score, surge_15m) moved to a global
    server-side config in Phase 2 (see `internal/config/config.go`
    `AmbushOIMCThreshold` / `AmbushZScoreThreshold` / `AmbushSurge15mThreshold`).
    The skill no longer sends a `trigger` block — server ignores per-bot
    thresholds and applies the deployment-wide values.
    """
    def _s(v):
        # Render decimals as fixed strings (no scientific notation for
        # small values like 0.05). Python's str(float) is fine here since
        # values are bounded in [0.01, 1.0].
        if isinstance(v, str):
            return v
...
class TradingClient:
    def __init__(self, api_key: str = "", api_secret: str = "",
        ...
        if parsed.path not in ("", "/"):
            raise ValueError(
                ...
                "'https://moss-dev.moss.site'. The client appends the API prefix "
                "automatically and will request "
                "'https://moss-dev.moss.site/api/v1/moss/agent/agents/bind' "
                "for bind."
            )

    @staticmethod
    def _normalize_symbol(symbol: str) -> str:
        """Normalize trading symbol: 'ETH/USDC' -> 'ETHUSDC', 'BTCUSDC' -> 'BTCUSDC'."""
    ...
    def create_realtime_bot(
        ...
        *,
        ...
        Ambush mode: pass strategy_type="ambush" + ambush_params={...}. The server
        accepts the V2 wire format where decimal fields are JSON strings (per
        CLAUDE.md "v2 wire format: decimal fields are JSON strings"). The skill
        side typically computes ambush_params from propose.py and just forwards.
        Majors mode: leave strategy_type empty (or "majors"); strategy_params is
        the legacy DecisionParams.
        """
    ...
    def get_supported_symbols(self) -> dict:
        """Fetch supported realtime majors (public, no auth)."""
        return self._request("GET", "/agent/supported-symbols", need_auth=False)
