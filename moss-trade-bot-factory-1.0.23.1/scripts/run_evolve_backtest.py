...
def _aggregate_trades_backend_style(fills: list) -> dict:
    ...
    for fill in fills:
        side = fill["side"]
        # core/backtest.py now stores qty/price as Decimal-safe strings, so D() can
        # consume them directly without a lossy float round-trip.
        qty = D(fill["qty"])
        price = D(fill["price"])
...
def main():
    ...
    merged_fill_equity_values = [args.capital]
    for r in seg_results:
        ...
        # Per-fill equity for Go-aligned sharpe/max_drawdown (one point per fill event,
        # matching backtest_replay_verify.go:verifyReplayEquityCurve).
        for f in getattr(r, "fills", []):
            if "equity_after" in f:
                merged_fill_equity_values.append(f["equity_after"])
    merged_equity_series = pd.Series(merged_equity_values, dtype=float)
    merged_fill_equity_series = pd.Series(merged_fill_equity_values, dtype=float)
    ...
    full_result = _build_result(
        ...
        fill_equity=merged_fill_equity_series,
    )
    ...
    backtest_result["total_return"] = float(full_result.total_return)
    backtest_result["sharpe_ratio"] = float(full_result.sharpe_ratio)
    backtest_result["max_drawdown"] = float(full_result.max_drawdown)
    backtest_result["win_rate"] = float(full_result.win_rate)
    backtest_result["profit_factor"] = float(full_result.profit_factor)
