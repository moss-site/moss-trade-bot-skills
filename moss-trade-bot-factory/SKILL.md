...
收到用户描述后**第一件事**是判断走主流币还是异动币。**按用户意图语义判断，不是字面关键词匹配**：

- → **Ambush（异动币）**：用户想做的是「监听小市值币 OI 异动事件、运行时才动态选币、不预先绑定具体币种」那类策略。典型表述：抢异动 / 异动小币 / 异动信号 / 埋伏小币 / 狙击异动 / 小市值币 / ambush，**且没有指定某个具体主流币**。
- → **主流币**：用户给了一个具体的主流币 symbol（22 个 USDC 永续或 21 个 xyz 类股票之一，如 BTC / ETH / SOL / HYPE / NVDA…）+ 回测区间 / 策略风格，想做的是**该指定币种**的择时策略。

> ⚠️ **「异动」是币圈高频普通词，单独出现要结合语境**：当它只是形容某个主流币的行情（例：「BTC 最近异动很大，帮我做个 BTC 趋势 bot」——用户已给具体主流币 symbol + 风格），这是**主流币**意图，**不要因为句中出现「异动」二字就路由到 Ambush**。只有当「异动」指代**策略类型本身**（抓小市值币 OI 异动、不绑定某个 symbol）时才走 Ambush。判断锚点是「有没有指定具体主流币 symbol」，不是「有没有出现某个词」。

两条流程互不交叉。一次只创建一个 bot；如果用户描述真的同时含两类**意图**，反问用户先创建哪个。
...
> 🔁 **进入前自检（防误入）**：如果用户消息里给了一个**具体主流币 symbol**（BTC / ETH / SOL / HYPE / NVDA 等 22+21 之一）并配了回测区间或择时风格，那意图其实是**主流币**——纵使句中出现「异动」这种高频普通词。此时**不要继续 Ambush**：回到主流币 Step 1-5，或反问用户「你是想做 BTC 这个具体币的择时策略，还是抓异动小币那类不绑币的策略？」。Ambush 只服务「抓小市值币 OI 异动、运行时动态选币、不绑某个 symbol」的意图。
