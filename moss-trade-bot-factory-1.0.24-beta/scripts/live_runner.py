...
Usage:
    ...
    # --platform-url should be site origin only, e.g. https://beta-agent.moss.site
...
RUNNING = True
PLATFORM_URL_HELP = "Platform site origin only, e.g. https://beta-agent.moss.site. The client appends API paths automatically."
...
def run_cycle(client: TradingClient, params: DecisionParams, timeframe: str,
    ...
    if not isinstance(positions, list):
        positions = []
    # Filter to only this bot's primary symbol. The platform's source_account
    # is per-bot but NOT per-symbol — any HMAC POST /orders against this
    # bot_id with a different symbol body would leave a cross-symbol position
    # in the same account (real example: a stale ETHUSDC short leaked into
    # a LINK bot's account from an earlier mis-routed order). Without this
    # filter, check_exit_conditions / open_long below run with a position
    # of one symbol and a mark_price of another, generating bogus
    # take_profit / stop_loss decisions and closing the wrong things.
    expected_sym = (client.symbol or "").upper()
    if expected_sym:
        positions = [
            p for p in positions
            if str(p.get("symbol", "")).upper() == expected_sym
        ]
