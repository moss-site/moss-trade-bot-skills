...
> **后端校验规则**：5 个权重之和必须 ≤ 1.0；只指定部分权重（partial patch）时后端把剩余预算 `1 - sum(explicit)` 平均分到未指定权重；提交全量参数（5 个全填）时严格 `sum == 1.0`，超 1.0 直接 `signal weights exceed total budget` 拒绝。生成参数前先确认这条约束。
