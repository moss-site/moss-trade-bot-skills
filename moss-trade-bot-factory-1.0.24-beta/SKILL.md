...
- **平台功能（可选）**：只有用户明确要求 upload / bind / live 时才连接外部平台。默认平台地址使用 skill config `trade_api_url`，默认值 `https://beta-agent.moss.site`
- **平台 URL 规则**：`--platform-url` 只填站点 origin，例如 `https://beta-agent.moss.site`；脚本会自动补上完整 API 前缀，并请求 `https://beta-agent.moss.site/api/v1/moss/agent/agents/bind`
